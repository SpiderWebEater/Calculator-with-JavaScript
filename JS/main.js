...
Calculator.prototype.selectPower = function (operation) {
    ...
    if (!this.expression) {
        if (
            operation.toString() === "√" ||
            operation.toString() === "x<sup>2</sup>"
        ) {
            console.log("in here");
...
Calculator.prototype.updateDisplay = function () {
    ...
    if (this.operation != null) {
        if (this.trigOperation !== null) {
            this.displayExpression.innerHTML = `${this.getDisplayNumber(
                this.expression
            )} ${this.operation}${this.trigOperation}`;
        } else {
            this.displayExpression.innerHTML = `${this.getDisplayNumber(
                this.expression
            )} ${this.operation}`;
...
let _ = document.querySelectorAll;
...
numbers.forEach(number => {
    number.addEventListener("click", function () {
        //all event handler function should not be in an arrow function because than they will lose >this <---
...
clearAll.addEventListener("click", function (button) {
    calculator.clearAll();
    calculator.updateDisplay();
});

prevdel.addEventListener("click", function (button) {
    calculator.delete();
    calculator.updateDisplay();
});
